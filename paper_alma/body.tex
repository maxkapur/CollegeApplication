...
\subsubsection{Related optimization problems}
...
\subsubsection{Admissions markets}
...
In the college application problem, the value $t_0$ of the noncollege option can function as a risk-aversion parameter. However, our analysis of the distributional heuristic reveals that regardless of their underlying risk preferences, students must modulate their risk allocation---that is, the relative share of reach and safety schools in their portfolio---according to the number of schools to which they can afford to apply. Students with a large application budget can tolerate, and indeed benefit, from taking larger risks than those without. This means that in college application, variation in students' risk allocation arises from a combination of variance in underlying preferences and variance in students' application budgets. This property, which we may call \emph{optimal risk elasticity,} is not present in the Markowitz model.

From a welfare standpoint, this optimal risk elasticity offers weak evidence for the notion that reducing the costs of college application could improve student equity. On the other hand, dissuading students from applying to too many reach schools may be a desirable feature in an admissions market inasmuch as it eliminates the administrative costs associated with reading low-quality applications. 
...
The fact that the optimal objective value grows sublinearly in $h$ may be relevant in the design of admissions markets. In the South Korean college admissions process, students are allowed to apply to only $h = 3$ colleges in the main application cycle, a policy which is thought to promote fairness. Suppose that utility is measured in monetary units, $t_0 = \$100$, and $v(\mathcal{X}_3) = \$130$ for a typical student. Then since $v(\mathcal{X}_4) \leq \$140$, a policy that increments the application limit will be unpopular if its tax cost exceeds $\$10$.

This also suggests, paradoxically, that colleges should offer application fee waivers to students who have already applied to many competitor schools. % interesting tradeoff

% you should increase $h$ until the point where the maximum marginal value of an additional application, namely $v(h) / h$, is small enough that students won't be tempted to spam reach schools with extra applications.
