...
Our analysis also provides a tight, constant upper bound on the marginal value of additional applications, which may be informative in the design of fee waiver policies and setting application limits in centralized admissions markets.
...
In this section, we discuss the implications of our model for real-world admissions markets. First, the optimality of the greedy algorithm together with the submodularity of the objective implies that $v(\mathcal{X}_h)$ grows sublinearly in $h$; we provide a tight example for this upper bound. Examining the student profiles for which $v(\mathcal{X}_h)$ grows most rapidly suggests that the underwhelming effectiveness of fee-waiver interventions may be explained by the fact that they target students who have little marginal benefit from additional applications. 

Next, we discuss the distributional heuristic, which targets a static risk distribution and (in its inoptimality) reveals that the college application problem possesses a unique property called optimal risk elasticity that is not present in traditional portfolio optimization problems. This property provides alternative explanation for social-science research showing that low-income students use a more risk-averse college application strategy than their wealthy peers.

\subsection{Risk management and student welfare}

Diminishing marginal returns are an intrinsic feature of submodular set functions. In the college application problem, this property, together with the optimality of the greedy algorithm, implies that the gain in student utility from increasing the budget from $h-1$ to $h$ is larger than that from increasing it from $h$ to $h+1$.
...
\begin{equation*}
v(\mathcal{X}_h) = \sum_{j= m-h +1}^m \Bigl( f_j t_j \prod_{i = j+1}^m (1 - f_{i}) \Bigr) =  \sum_{j= 1}^h  (1 - \varepsilon)^{m-h+j}.
\end{equation*}
Thus, as $\varepsilon$ approaches zero, we have $v(\mathcal{T}_h) \to h$.
\end{example}
...
Example \ref{tightexampleforoneoverhopt}, in which $v(\mathcal{X}_h)$ is $O(h)$, demonstrates that the objective function of the college application problem approaches linearity when the admissions probabilities $f_j$ are very small. In the knapsack-constrained college application problem, Kapur \cite{kapur2022} exploits this property to reduce the problem to the binary knapsack problem and establish its NP-completeness.
