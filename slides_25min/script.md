...
저희가 알기로는 이 문제를 체계적으로 분석하는 것은 처음입니다. 문제를 한 문장으로 요약하자면, 예산 제약식 하에서, 여러 확률 변수를 구매하는 문제인데, 그것의 기대 최댓값을 최대화하는 문제입니다. 'max'라는 말이 두번 나타나는데, 이것 때문에 문제의 수리 모형이 조금 독특해집니다. 오늘의 발표에서는, 이 최적화 문제의 수리 모형 그리고 이 모형 어떻게 대학 지원 전략에 적용할 수 있는지 먼저 설명해드리겠습니다. 그다음에, 이 문제에 대한 이론적인 성질 그리고 해법 알고리즘을 탐구한 저희의 연구 결과를 정리해드리고자 하겠습니다.
