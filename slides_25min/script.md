...
다항 시간에 풀 수 있는 특수한 경우인 알마의 문제를 먼저 보겠습니다. 이것은 아까 본 그림처럼 배낭 제약식 대신 지원 포트폴리오의 크기만 제한되는 경우이며, 지원 비용은 없지만 정시 모집 기간에 학교 3개만 지원할 수 있는 한국 입학 과정과 비슷한 상황입니다. 이 케이스에서, $H$는 지원하는 지출액이 아닌 학교의 개수를 의미하므로, $h$가 $m$보다 작다고 가정할 수 있고, 그것을 강조하기 위해서 소문자로 표기하겠습니다. 알마의 문제는 집합 크기 제약 위에서 submodular 함수를 최적화하는 문제이고, 고전적인 결과에 따라 탐욕이 좋은 근사 해법이 되는 것이 알려져 있습니다. 그런데, 대학 지원 문제에 대해서, 저희는 같은 탐욕 해법이 실제로 정확한 해법이 된다는 좀 더 강한 결과를 보일 수 있었으며, 그 해법의 계산 시간고 줄일 수 있었습니다.

The optimality of the greedy algorithm for Alma's problem stems from the fact that the optimal portfolios have a special structure: When the application limit increases from $h$ to $h+1$, the optimal application portfolio is formed by just adding one school to the one before it. We prove this so-called "nestedness" property using a novel variable-elimination technique. Recycling this technique enables us to compute the objective function at each iteration in to amortized unit time. This brings the overall computation time down to a quadratic.

알마의 문제에서 탐욕 해법이 정확한 해법이 되는 이유는, 최적 포트폴리오가 특별한 구조를 가지기 때문입니다. 지원 제한이 $h$에서 $h+1$로 늘어날 때, 새로운 최적 포트폴리오는 그 전의 최적해의 확장 집합입니다. 이러한 '포함 사슬 관계', 즉 nestedness 성질을 어떤 새로운 변수 소거법을 개발하여 증명할 수 있었습니다. 알고리즘의 구현의 이 변수 소거법을 다시 활용해서 목적 함수를 계산하는 시간을 $O(m)$에서 환산 단위 시간으로 줄였습니다. 따라서 전체 알고리즘을 돌리는 것은 이차 시간을 넘지 않습니다.
