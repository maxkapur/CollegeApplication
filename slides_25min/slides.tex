...
\looseness=-1
일반적으로, 배낭 제약식 위에서 submodular 함수를 최대화하는 문제는 $(1 - 1/e)$보다 좋은 근사 계수를 가지는 해법이 존재하지 않으므로 (Kulik et al. 2013), 엘리스 문제에 FPTAS가 존재하는 것은 의미가 있는 결과다.
\end{frame}
} \fi
...
\else
이제 $G[j, v]$-값을 표에 채우면 근사 최적 포트폴리오를 구할 수 \rlep{있다.}
