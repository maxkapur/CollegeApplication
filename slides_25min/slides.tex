...
예산 제약 조건 하에서, 다수 확률 변수로 이루어진 포트폴리오의 \textbf{기대 최댓값을 최대화}하는 문제다.
...
\begin{frame}{효용 모형}
학교 $c_j$에 다니면 $t_j \geq 0$ 단위의 \textbf{효용이} 발생한다. Wlog, $t_j \leq t_{j+1}$.
...
일반적으로, 배낭 제약식 위에서 submodular 함수를 최대화하는 문제는 $(1 - 1/e)$보다 좋은 근사 계수를 가지는 해법이 존재하지 않으므로 (Nemhauser와 Wolsey 1978; Kulik et al. 2013), 엘리스 문제에 FPTAS가 존재하는 것은 의미가 있는 결과다.
\end{frame}
} \fi
...
Then $H$ is simply a limit on the number of schools you can apply to. This case mirrors the main Korean admissions cycle, in which $h = 3$ and $m \approx 200$. 
...
``알마의 문제''라는 \textbf{특수한 경우}에서 모든 $g_j = 1$이다.

이때 $H$는 단순한 지원 개수 제한이 되며, $h = 3, m \approx 200$인 한국 정시 입학 과정과 비슷한 상황이다.

(이 경우, $H < m$임을 가정할 수 있으므로 모든 $H$의 다항식에 대해 $m$의 다항식인 상한이 존재한다. 이를 강조하기 위해 $h$처럼 소문자로 표기한다.)

집합 크기 제약 위에 단조 submodular 함수를 최대화하는 문제에 대해, 탐욕 해법의 근사 계수가 $(1 - 1/e)$임이 알려져 있다 (Fisher 외 1978). 대학 지원 문제에 대해, 같은 해법이 정확한 해법임을 증명하고 계산 시간을 줄일 수 있다.
\end{frame}
} \fi
...
학교 $f_j = \varepsilon$으로 넣으면 $v(\mathcal{X})$를 선형 함수에 원하는 만큼 가깝게 만들 수 있다. 이는 배낭 문제가 엘리스의 문제로 변환할 수 있음을 의미하므로 엘리스의 문제는 NP-complete하다.

4개의 알고리즘 제시:
\begin{itemize}
\item 선형 완화 문제와 해당 \textbf{분지한계} (branch-and-bound) 해법. 일반적인 INLP 문제에 대해 자주 쓰이는 방법이다.

\item \textbf{총 지원 비용 기반 동적 계획}. $O(Hm + m\log m)$ (의사 다항) 시간에 정확한 해를 구하며, $g_j$가 작은 정수인 전형적인 인스턴스에 대해 매우 효율적인 해법.

\item 포트폴리오 가치의 라운딩을 이용한 동적 계획. $O(m^3 / \varepsilon)$ 시간에 $(1 - \varepsilon)$-근사해를 출력하므로 \textbf{FPTAS!}

\item \textbf{Simulated annealing} 휴리스틱. 빠르고 대부분 98\% 이상의 최적성을 얻는다.
\end{itemize}
...
\item \textbf{총 지원 비용 기반 동적 계획}. $O(Hm + m\log m)$ (의사 다항) 시간에 정확한 해를 구하며, $g_j$가 작은 정수가 되는 전형적인 인스턴스에 대해 매우 효율적인 해법.
...
정확도 보장은 없지만, 계산 시간이 아주 짧다. ⇒ 차량 라우팅처럼, ``충분히 좋은'' 해가 실시간으로 필요한 분야에서 인기가 많다.

SA의 구성 요소:
\begin{itemize}
\item \textbf{초기 해}. 본 연구에서 선형화 휴리스틱을 적용하고 해당 배낭 문제를 탐욕 휴리스틱으로 풀어서 초기 해를 구한다.

\item \textbf{해의 이웃해를 생성하는} 확률적 방법. $\mathcal{X}$가 가능해가 아닐 때까지 학교를 무작위로 추가하고, 다시 가능해가 되도록 무작위로 빼는 방법을 사용한다.

\item 해법이 이웃해로 바꾸는 확률을 결정하는 \textbf{온도 모수}. Grid 탐색을 적용한 결과로 $T =1/4$ 그리고 $r=1/16$을 선택했다.
\end{itemize}
} \fi
\end{frame}
...
\ifen 
 \textbf{\begin{tabular}[r]{@{}r@{}}\\$m$\end{tabular}}& \multicolumn{2}{c|}{\textbf{\begin{tabular}[c]{@{}c@{}}Greedy alg.\\with array\end{tabular}}}  & \multicolumn{2}{c}{\textbf{\begin{tabular}[c]{@{}c@{}}Greedy alg.\\with heap\end{tabular}}}\\ \hline
 \else
 \textbf{\begin{tabular}[r]{@{}r@{}}\\$m$\end{tabular}}& \multicolumn{2}{c|}{\textbf{\begin{tabular}[c]{@{}c@{}}탐욕 알고리즘\\(배열 구현)\end{tabular}}}  & \multicolumn{2}{c}{\textbf{\begin{tabular}[c]{@{}c@{}}탐욕 알고리즘\\(힙 구현)\end{tabular}}}\\ \hline
 \fi
...
\textbf{향후 연구:}
\begin{itemize}
\item 고전적 자산배분 모형(Markowitz 1952)처럼, 위험 회피를 명시적으로 다루는 새로운 목적함수.
...
\xrowht[()]{1.7em}  \begin{tabular}[r]{@{}r@{}}분\end{tabular}   & 일반 문제          & 없음                  & 정확                    & $O(2^m)$                  \\
