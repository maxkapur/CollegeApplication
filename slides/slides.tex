...
\documentclass[10pt,slidestop,compress,mathserif,notheorems]{beamer}
...
\setbeamertemplate{footline} {
  ...
  \hspace*{1.5ex}\insertshortauthor, \insertdate
  \end{beamercolorbox}%
  \begin{beamercolorbox}[wd=.15\paperwidth,ht=2.65ex,dp=1.2ex,left]{date in head/foot}%
  \hspace*{1ex}
    \ifnum \theframenumber=1{}
    \else \insertshorttitle%~\textbar~\insertsectionhead
    \fi
     \end{beamercolorbox}%
  \begin{beamercolorbox}[wd=.35\paperwidth,ht=2.65ex,dp=1.2ex,right]{date in head/foot}%
    \ifnum \theframenumber=1{}
    \else \insertsectionhead
    \fi
  \hspace*{2ex}
     \end{beamercolorbox}%
  \begin{beamercolorbox}[wd=.25\paperwidth,ht=2.65ex,dp=1.2ex,right]{author in head/foot}%
...
%\begin{itemize}
%\item A branch-and-bound routine.
%\item Dynamic program based on expenditures.
%\item Dynamic program based on portfolio valuations, yielding an FPTAS.
%\end{itemize}
...
%\begin{itemize}
%\item 분지한계법.
%\item 지출액 기반 동적 계획 해법.
%\item 포트폴리오 가치 기반 동적 계획 해법을 이용하는 FPTAS.
%\end{itemize}
...
\end{center}
\ifen
Simulated annealing heuristic: details unremarkable. See next slide for an accuracy plot.
\else
