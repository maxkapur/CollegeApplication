...
\item \textbf{배낭 문제}의 일반화: 정수 패킹 제약식, NP-completeness, 근사 해법.
...
어떤 대학에도 합격 안 하면 효용이 $t_0$이며, wlog $t_0 =0$임을 가정할 수 있다 (논문에서 증명).
...
\begin{frame}{기존의 해법}
입학 컨설팅 산업에서는 주로 ``상향·소신·안정 지원 학교'' 각각 균일하게 지원하는 \textbf{배분적 휴리스틱}을 사용하며, \textbf{위험 회피적인} 전략임을 알 수 있다. 

또한 \textbf{선형화 휴리스틱}이 있다. $c_j$에만 지원할 때 기대 효용이 $f_j t_j$이므로 다음 배낭 문제를 대리 문제로 푼다.
\[\text{maximize}~~\sum_{j\in \mathcal{X}} f_j t_j \qquad \text{subject to}~~\sum_{j\in \mathcal{X}}g_j \leq H \]
이의 \textbf{근사 계수가 $\mathbf{0}$에 무한히 가까운} 예시를 만들 수 있다.

Fu (2014)는 비슷한 문제를 \textbf{열거법}으로 풀었으며, $m \geq 20$일 때 불현실적인 방법이다.

본 연구가 제시하는 알고리즘은 \textbf{계산 시간과 정확도를 보장한다}. 
\end{frame}
} \fi
...
\begin{frame}{Conclusion}
``Maximax'' 형태와 정수 조건 때문에 대학 지원 문제에는 \textbf{이론적으로 흥미}가 있다. Submodular 최대화 문제이지만, 근사 해법 성질은 배낭 문제에 더 가깝다.
...
문제를 확장할 수 있는 향후 연구: 위험 회피 모수, 배분적 (가나다군) 제약 조건, 메모리 소모량 절감.
\end{frame}
} \fi
